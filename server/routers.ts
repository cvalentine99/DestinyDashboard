...
const achievementsRouter = router({
  ...
  getDefinitions: publicProcedure.query(() => {
    return {
      ...
      categories: ['combat', 'boss', 'flawless', 'score', 'class', 'weapon', 'survival', 'collection', 'special'],
  ...
  getProgress: protectedProcedure.query(async ({ ctx }) => {
    ...
    const progress = Object.values(ACHIEVEMENT_DEFINITIONS).map(def => {
      ...
      if (def.statKey === 'special') {
        currentValue = calculateSpecialAchievementProgress(def.id, stats as unknown as Record<string, number>);
      } else if (def.statKey && stats[def.statKey as keyof typeof stats] !== undefined) {
        currentValue = Number(stats[def.statKey as keyof typeof stats]) || 0;
      }
      
      const isCompleted = userAch?.isCompleted || currentValue >= def.targetValue;
      const progressPercent = Math.min(100, (currentValue / def.targetValue) * 100);
  ...
  getNotifications: protectedProcedure.query(async ({ ctx }) => {
    const notifications = await db.getPendingAchievementNotifications(ctx.user.id);
    return notifications.map(n => {
      const def = ACHIEVEMENT_DEFINITIONS[n.achievementId as keyof typeof ACHIEVEMENT_DEFINITIONS];
      return {
        ...n,
        achievement: def ? {
          ...def,
  ...
  submitGameStats: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const totalKillsThisGame = input.dregKills + input.vandalKills + input.captainKills +
        input.thrallKills + input.acolyteKills + input.knightKills +
        input.goblinKills + input.hobgoblinKills + input.minotaurKills +
        input.ogreKills + input.servitorKills + input.hydraKills;
      ...
      const newStats = {
        ...
        rocketLauncherKills: (stats.rocketLauncherKills || 0) + input.rocketLauncherKills,
        ...
        cosmodromeClears: (stats.cosmodromeClears || 0) + (input.clearedCosmodrome ? 1 : 0),
        europaClears: (stats.europaClears || 0) + (input.clearedEuropa ? 1 : 0),
        dreamingCityClears: (stats.dreamingCityClears || 0) + (input.clearedDreamingCity ? 1 : 0),
      ...
      for (const def of Object.values(ACHIEVEMENT_DEFINITIONS)) {
        ...
        let currentValue = 0;
        if (def.statKey === 'special') {
          currentValue = calculateSpecialAchievementProgress(def.id, updatedStats as unknown as Record<string, number>);
        } else if (def.statKey && updatedStats[def.statKey as keyof typeof updatedStats] !== undefined) {
          currentValue = Number(updatedStats[def.statKey as keyof typeof updatedStats]) || 0;
        }
        
        // Update progress
        const isNowCompleted = currentValue >= def.targetValue;
        await db.updateAchievementProgress(userId, def.id, currentValue, isNowCompleted);
...
// Loadout router for saving weapon/class combinations
const loadoutRouter = router({
  ...
  create: protectedProcedure
    .input(z.object({
      ...
      guardianClass: z.enum(["titan", "hunter", "warlock"]),
      primaryWeapon: z.enum(["auto_rifle", "hand_cannon", "pulse_rifle", "rocket_launcher"]),
  ...
  update: protectedProcedure
    .input(z.object({
      ...
      primaryWeapon: z.enum(["auto_rifle", "hand_cannon", "pulse_rifle", "rocket_launcher"]).optional(),
      iconColor: z.string().optional(),
    }))
    .mutation(async ({ ctx, input }) => {
      const loadout = await db.getLoadoutById(input.id);
...
function getClassColor(guardianClass: string): string {
  switch (guardianClass) {
    case "titan": return "#f97316"; // Orange for Titans
    case "hunter": return "#06b6d4"; // Cyan for Hunters
    case "warlock": return "#a855f7"; // Purple for Warlocks
    default: return "#00d4aa"; // Teal default
  }
}
